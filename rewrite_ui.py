...
with open('frontend/lib/screens/horoscope_info_screen.dart', 'r', encoding='utf-8') as f:
    content = f.read()
...
content = re.sub(r'  Widget _buildFormRow.*Widget _buildSectionTitle.*?Widget build\(BuildContext context\) {', new_methods + '\n  @override\n  Widget build(BuildContext context) {', content, flags=re.DOTALL)

body_start = content.find('body: Center(')
if body_start != -1:
    ...
    content = content[:body_start] + new_body + content[body_end + len('        ),\n      );\n  }\n}\n'):]

with open('frontend/lib/screens/horoscope_info_screen.dart', 'w', encoding='utf-8') as f:
    f.write(content)

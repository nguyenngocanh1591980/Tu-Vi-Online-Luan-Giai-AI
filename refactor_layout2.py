...
def replace_field(field_name, content_str, is_wrap=False):
    ...
    if not match:
        print('Not found title:', field_name)
        return content_str
    ...
    wrap_pattern = r"Padding\(\s*padding:.*?child:\s*(?:Wrap|Column)\(.*?\s*children:\s*\["
    
    if is_wrap:
        match2 = re.search(wrap_pattern, content_str[end_title:], flags=re.DOTALL)
    else:
        match2 = re.search(row_pattern, content_str[end_title:], flags=re.DOTALL)
        
    if not match2:
        print('Not found row/wrap:', field_name)
        return content_str
